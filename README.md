...
全局代理模式下，所有流量（包括飞书、百度等国内服务）都会绕道海外代理节点再回国内。这会导致：
- 飞书等国内服务因"出国又回国"被安全网关拦截，无法连接
- 国内服务访问速度大幅下降

智能分流模式（rule）自动将国内流量直连、海外流量走代理。每个域名固定走一条路线，不存在 IP 切换问题。配置中已内置飞书、钉钉、百度、微信等 30+ 常用国内域名的显式直连规则，并以 GEOIP 兜底。

### 防失联保留方案

代理环境下最大的风险是节点失效导致 AI 助手与用户失联。本项目内置了三层保护：

1. **守护进程（watchdog）** — 后台每 60 秒通过代理访问 gstatic.com（海外站点）检测代理节点是否可用，失败时自动切换节点；如果所有节点都挂了，自动刷新订阅拉取最新节点再试
...
配置完成后，AI 助手会将以下关键信息写入它自己的 `MEMORY.md` 持久记忆，确保跨会话也不会忘记：

1. 无头浏览器必须传 `--proxy-server` 代理参数（不读环境变量）
...
bash scripts/switch-mode.sh rule              # 切智能分流（默认）
