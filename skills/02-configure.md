...
> 你希望用哪种连接模式？
>
> 1. **智能分流（推荐）** - 国内网站（飞书、百度等）直连，海外网站（GitHub、Google）走代理。每个域名固定走一条路，不会 IP 切换
> 2. **全局代理** - 所有流量都走代理（注意：会导致飞书等国内服务连不上）
> 3. **不确定** - 我帮你选智能分流

根据用户回答：
- 智能分流 / 不确定 → 使用默认配置（mode: rule），无需额外操作
- 全局代理 → 在第 3 步启动后执行 `bash scripts/switch-mode.sh global`
